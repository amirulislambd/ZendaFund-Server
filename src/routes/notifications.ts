...
// Accept POST as a fallback for clients that send POST instead of PATCH
router.post("/notifications/:id/read", verifyToken, async (req: AuthRequest, res) => {
  try {
    const id = req.params.id;
    if (!id || Array.isArray(id)) return res.status(400).json({ success: false, message: "Invalid id" });
    if (!ObjectId.isValid(id)) return res.status(400).json({ success: false, message: "Invalid id" });
...
// Accept POST as a fallback for clients that send POST instead of DELETE
router.post("/notifications/:id", verifyToken, async (req: AuthRequest, res) => {
  try {
    const id = req.params.id;
    if (!id || Array.isArray(id)) return res.status(400).json({ success: false, message: "Invalid id" });
    if (!ObjectId.isValid(id)) return res.status(400).json({ success: false, message: "Invalid id" });
